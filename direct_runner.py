...
SYSTEM = """You are a code generation agent in a controlled cybersecurity research environment.
...
def run_direct(model: str, prompt_key: str, workspace: Path) -> dict:
    print(f"\n{'='*60}")
    print(f"DIRECT PROMPT | Model: {model} | Prompt: P{prompt_key}")
    print(f"{'='*60}")
    ...
    planner_result = planner_node(state) or {}
    state.update(planner_result)

    state["current_code"] = code
    state["iteration"] = 1
    state["messages"].append("Direct prompt generation completed.")

    review_result = reviewer_node(state) or {}
    state.update(review_result)


    evasion_result = evasion_analyst_node(state) or {}
    state.update(evasion_result)
...
def run_batch(workspace: Path):
    ...
    for model in MODELS:
        for prompt_key in ["1", "2", "3", "4"]:
            try:
                summary = run_direct(model, prompt_key, workspace)
    ...
    (workspace / "direct_batch_results.json").write_text(json.dumps(results, indent=2))
...
if __name__ == "__main__":
    ...
    workspace = Path("workspace_direct2")
    workspace.mkdir(exist_ok=True)

    if args.batch:
        run_batch(workspace)
    else:
        run_direct(args.model, args.prompt, workspace)